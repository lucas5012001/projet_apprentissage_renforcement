            ...
            for j in heights:
                for k in sqs:
                    for l in what:
                        ...
                        self.states[str((i,j,k,l))] = [random.uniform(-1, 1) for _ in range(4)]
...
def experience_multi(N, Nreps, epsilon, lr, discount, reward_game_over, reward_food, reward_bomb, reward_good_move, reward_bad_move, Ntest=200):
    results = {}
    results_Q = {}
    pool = Pool(processes=(cpu_count()-1))
    for i in range(N):
        async_result = pool.apply_async(run_experiment, args=(Nreps, epsilon, lr, discount, reward_game_over, reward_food, reward_bomb, reward_good_move, reward_bad_move, Ntest))
        result = async_result.get()  # Attendre que le résultat soit disponible
        results[str(i)] = result[0]
        results_Q[str(i)] = result[1]
    pool.close()
    pool.join()
    best_key = max(results, key=results.get)
    return results_Q[best_key]
...
def run_experiment(Nreps, epsilon, lr, discount,reward_game_over,reward_food,reward_bomb,reward_good_move,reward_bad_move,Ntest = 200):
    ...
    while game_count <= Ntest:
        ...
        scores.append(score)
        game_count += 1
    return np.mean(scores), learner.qvalues

if __name__ == "__main__":
    start_time = time.time()
    res = experience_multi(N=N,Nreps=Nreps,
                epsilon=epsilon,
                lr=lr,
                discount=discount,
                reward_game_over=reward_game_over,
                reward_food=reward_food,
                reward_bomb=reward_bomb,
                reward_good_move=reward_good_move,
                reward_bad_move=reward_bad_move,
                Ntest=Ntest)
    end_time = time.time()

    filename = "Q_matrix.json"
